...
- **Xception** — same reasoning category (spatial texture); EfficientNet's compound scaling gives a better accuracy/parameter tradeoff, and the research doc's §2 flags EfficientNet as the "cheapest strong option." Running both would repeat the two-backbone redundancy problem at smaller scale.
