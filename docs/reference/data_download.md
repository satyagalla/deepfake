...
Prep-phase only — must finish **before** the 1.5hr training clock starts (per `001_architecture_decisions.md`). Goal: three class folders of face-cropped images plus a manifest, ready for the `Dataset`/`DataLoader` described in `model_code.md`.
...
- **Real:** the pristine images paired 1:1 with that same DALL-E 3 slice (not an unrelated curated set — see `001_architecture_decisions.md`'s Dataset section for why).
