...
Implements the 3-branch fusion architecture from `0001-architecture-decisions.md`. Reads from the `train/val` folders and `manifest.csv` produced per `data_download.md`.
...
Confirm the assigned GPU immediately. Everything below assumes the A100 40GB path from `0001-architecture-decisions.md`'s GPU-tier table. If a T4 or L4 is assigned instead, switch to that table's fallback config (smaller backbone, partial freeze, stacked-channel input) rather than attempting the full 3-branch version under time pressure.
...
- The gate's per-branch contribution weights for those same samples, reported alongside the Grad-CAM heatmap — the paired output (heatmap + branch-contribution percentages) is the explainability deliverable described in `0001-architecture-decisions.md`'s overview diagram; don't report one without the other.

## Time budget checkpoints (A100 path, from 0001-architecture-decisions.md)
