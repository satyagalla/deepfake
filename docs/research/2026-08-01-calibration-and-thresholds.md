...
>
> **Note added 2026-08-02 — [`../decisions/0005-measurement-and-verdict-semantics.md`](../decisions/0005-measurement-and-verdict-semantics.md) resolves this doc against the v2 build, splitting it in two.** Findings unchanged; what changed is which of them get built.
> - **Built.** §2's threshold-placement result is the spine of the v2 metric set (`0005` §4): AUC as the primary metric and the discriminator between a misplaced threshold and lost separability; the label-free `α = E[z]` correction reported alongside the default cut; §1.2's base-rate math implemented as `precision(π)` derived from prevalence-invariant TPR/FPR (`0005` §4.3). §1.2 also turned out to be a **live defect** in the headline experiment, not just theory — `0005` §5.
> - **Deliberately not built.** §1.1/§1.3's calibration chain. The v2 build removes its Platt stage outright and emits no calibrated probability (`0005` §3, §8): the calibrator was fitted on COCO_AI val and applied to distributions the live test occupies, and ECE on tens of AI images per generator would be noise. **This is the one place this doc's ambition exceeds the data available to the build** — stated there rather than quietly skipped. §3's multicalibration point survives as a talking point for the same reason.
