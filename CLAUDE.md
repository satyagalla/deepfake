...
- Investigated the severe blur visible in all three demo screenshots (`docs/investigations/2026-07-26-upscale-artifact.md`): confirmed it's MTCNN's unconditional bilinear crop-resize amplifying small detected face boxes, measured at a similar ~10-12x median upscale factor across `real`, `deepfake`, and `edited` alike (not a clean per-class blur-magnitude confound, as initially assumed before `edited` and `real` were actually measured) — open hypothesis was a subtler interaction between that shared resampling artifact and each class's underlying pixel statistics
- Tested that hypothesis against the trained checkpoint with two counterfactual probes: `model/counterfactual_probe.py` (swaps fft/srm templates only, `docs/investigations/2026-07-27-fft-srm-template-swap-probe.md`) rules out a targeted per-class fft/srm shortcut but never varies `rgb`; the follow-up `model/resolution_swap_probe.py` (varies `rgb` directly, `docs/investigations/2026-07-27-resolution-swap-probe.md`) rules out a *general* resolution-magnitude shortcut across 2 of 3 achievable class pairs, replicated at a 4x larger upscale-factor remeasurement (n=200) — the third pair (`real->edited`) shows a small, reproducible effect that the remeasurement shows is not explained by perturbation magnitude alone, left as an open (small) finding, not further pursued

**Next:**

- Address the already-known cross-generator generalization failure (README: confidently wrong on Gemini/gpt-image-1 images) — the shortcut-learning hypothesis didn't turn up a general resolution-magnitude cue, so this is no longer blocked on disambiguating that first
- Optional, lower priority: a finer dose-response sweep isolating the unresolved small `real->edited` effect from `2026-07-27-resolution-swap-probe.md`, if it's ever worth pinning down further
